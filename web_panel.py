...
import httpx
from flask import Flask, request, redirect, url_for, session, render_template_string, flash, send_file, jsonify
...
<style>
:root{--bg:#07111f;--surface:#0c1929;--surface2:#101f33;--line:#203a56;--text:#eef6ff;--muted:#8ea5bd;--primary:#35d49a;--primary2:#39a9ff;--danger:#ff5d76;--warn:#ffc15d;--shadow:0 20px 60px rgba(0,0,0,.28);--radius:20px}
*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;min-height:100vh;background:radial-gradient(circle at 80% -10%,rgba(57,169,255,.18),transparent 28%),radial-gradient(circle at 10% 0%,rgba(53,212,154,.12),transparent 24%),var(--bg);color:var(--text);font-family:Tahoma,Arial,sans-serif}
a{color:inherit}.app{display:grid;grid-template-columns:250px minmax(0,1fr);min-height:100vh;transition:grid-template-columns .22s ease}.sidebar{position:sticky;top:0;height:100vh;height:100dvh;min-height:0;padding:16px 12px;display:flex;flex-direction:column;overflow:hidden;background:rgba(7,17,31,.94);border-left:1px solid var(--line);backdrop-filter:blur(18px);z-index:30;box-shadow:-12px 0 40px rgba(0,0,0,.16);transition:width .22s ease,padding .22s ease}.brand{display:flex;align-items:center;gap:11px;padding:6px 8px 15px;border-bottom:1px solid var(--line);margin-bottom:8px;flex:0 0 auto;min-width:0}.logo{width:46px;height:46px;flex:0 0 46px;border-radius:15px;display:grid;place-items:center;background:linear-gradient(135deg,var(--primary2),var(--primary));box-shadow:0 10px 35px rgba(57,169,255,.24);font-size:23px}.brand h1{font-size:20px;margin:0;white-space:nowrap}.version{display:block;color:var(--muted);font-size:11px;margin-top:3px;white-space:nowrap}.side-label{font-size:10px;color:#67809a;padding:8px 10px;flex:0 0 auto}.nav{display:flex;flex-direction:column;gap:5px;overflow-y:auto;overflow-x:hidden;min-height:0;flex:1 1 auto;padding:2px 2px 10px;scrollbar-width:thin;scrollbar-color:#31506d transparent}.nav::-webkit-scrollbar{width:6px}.nav::-webkit-scrollbar-track{background:transparent}.nav::-webkit-scrollbar-thumb{background:#31506d;border-radius:99px}.nav a{text-decoration:none;color:#cfe0f1;padding:11px 12px;border-radius:13px;border:1px solid transparent;display:flex;align-items:center;gap:9px;transition:background .18s,border-color .18s,transform .18s,color .18s; font-size:13px;flex:0 0 auto;white-space:nowrap;position:relative;overflow:hidden;isolation:isolate}.nav a::after{content:"";position:absolute;right:12px;left:12px;bottom:4px;height:2px;border-radius:99px;background:linear-gradient(90deg,var(--primary2),var(--primary));transform:scaleX(0);transform-origin:right center;opacity:0;transition:transform .22s ease,opacity .22s ease;pointer-events:none}.nav a:hover{background:var(--surface2);border-color:var(--line);transform:translateX(-2px)}.nav a:hover::after,.nav a.active::after{transform:scaleX(1);opacity:.95}.nav a.active{background:linear-gradient(135deg,rgba(57,169,255,.18),rgba(53,212,154,.12));border-color:#285273;color:#fff}.nav a .ripple{position:absolute;border-radius:50%;pointer-events:none;transform:scale(0);opacity:.5;background:radial-gradient(circle,rgba(255,255,255,.55) 0%,rgba(57,169,255,.32) 38%,rgba(57,169,255,0) 72%);animation:navRipple .55s ease-out forwards;z-index:0}.nav a>span,.nav a:not(:has(span)){position:relative;z-index:1}.nav a>span{z-index:1}@keyframes navRipple{to{transform:scale(1);opacity:0}}.side-bottom{position:static;display:flex;flex-direction:column;gap:8px;padding-top:9px;border-top:1px solid var(--line);flex:0 0 auto;background:rgba(7,17,31,.94)}.logout{display:block;text-align:center;text-decoration:none;padding:10px 12px;border:1px solid var(--line);background:var(--surface2);border-radius:12px;color:#dbe8f5;font-size:13px}.sidebar-toggle{position:absolute;top:14px;left:10px;width:34px;height:34px;border:1px solid var(--line);background:var(--surface2);color:var(--text);border-radius:10px;cursor:pointer;font-size:17px;display:grid;place-items:center;z-index:2}.sidebar-collapsed .app{grid-template-columns:76px minmax(0,1fr)}.sidebar-collapsed .sidebar{padding-left:8px;padding-right:8px}.sidebar-collapsed .brand{display:none}.sidebar-collapsed .side-label{display:none}.sidebar-collapsed .nav{padding-top:50px}.sidebar-collapsed .nav a{justify-content:center;padding-left:8px;padding-right:8px}.sidebar-collapsed .nav a span{display:none}.sidebar-collapsed .side-bottom{display:none}.sidebar-collapsed .sidebar-toggle{left:21px}.sidebar-overlay{display:none}.cursor-effects{position:fixed;inset:0;width:100vw;height:100vh;pointer-events:none;z-index:9999}.cursor-effects{mix-blend-mode:screen} .icon-btn{position:relative;background:var(--surface2);color:var(--text);border:1px solid var(--line);border-radius:11px;padding:9px 12px;cursor:pointer}.notif-btn b{position:absolute;top:-6px;left:-5px;min-width:18px;height:18px;border-radius:99px;background:var(--danger);color:white;font-size:10px;display:grid;place-items:center}.command-palette,.notify-panel{position:fixed;z-index:10050;background:rgba(9,22,38,.98);border:1px solid #315574;box-shadow:0 25px 80px rgba(0,0,0,.45);backdrop-filter:blur(18px);border-radius:18px}.command-palette{top:12%;left:50%;transform:translateX(-50%);width:min(650px,92vw);padding:14px;display:none}.command-palette.show{display:block}.command-palette input{margin:0}.cmd-results{max-height:55vh;overflow:auto;margin-top:8px}.cmd-results a{display:block;padding:12px;border-radius:11px;text-decoration:none}.cmd-results a:hover{background:var(--surface2)}.notify-panel{top:68px;left:24px;width:min(390px,90vw);display:none;padding:14px}.notify-panel.show{display:block}.notify-item{padding:11px;border-bottom:1px solid var(--line);font-size:12px}.notify-item:last-child{border:0}.dashboard-widget{cursor:grab}.dashboard-widget.dragging{opacity:.45}.skeleton{position:fixed;inset:0;z-index:10040;background:rgba(7,17,31,.35);pointer-events:none;opacity:0;transition:.15s}.skeleton.show{opacity:1}.skeleton:after{content:'';position:absolute;top:0;left:0;width:35%;height:3px;background:linear-gradient(90deg,transparent,var(--primary2),var(--primary),transparent);animation:loadline 1s infinite}@keyframes loadline{from{transform:translateX(-120%)}to{transform:translateX(350%)}}
.main{min-width:0}.topbar{height:76px;display:flex;align-items:center;justify-content:space-between;gap:15px;padding:0 28px;border-bottom:1px solid rgba(32,58,86,.7);background:rgba(7,17,31,.45);backdrop-filter:blur(14px);position:sticky;top:0;z-index:8}.top-title{font-size:15px;font-weight:800}.top-sub{font-size:11px;color:var(--muted);margin-top:4px}.top-actions{display:flex;gap:8px;align-items:center}.pill{padding:8px 11px;border-radius:999px;background:var(--surface2);border:1px solid var(--line);font-size:11px;color:#bcd0e2}.content{max-width:1500px;margin:auto;padding:28px 30px 50px}.hero{position:relative;overflow:hidden;background:linear-gradient(135deg,rgba(57,169,255,.17),rgba(53,212,154,.11));border:1px solid #2b4e6b;border-radius:24px;padding:27px;box-shadow:var(--shadow);margin-bottom:18px}.hero:after{content:"";position:absolute;width:180px;height:180px;border-radius:50%;left:-55px;top:-80px;background:rgba(53,212,154,.08);filter:blur(5px)}.hero h2{margin:0 0 7px;font-size:25px}.hero p{margin:0;color:var(--muted);line-height:1.8}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:14px}.statcard{position:relative;overflow:hidden;background:linear-gradient(180deg,rgba(16,34,58,.96),rgba(10,24,40,.96));border:1px solid var(--line);border-radius:19px;padding:18px;box-shadow:var(--shadow);transition:.2s}.statcard:hover{transform:translateY(-3px);border-color:#315574}.staticon{font-size:23px}.statlabel{color:var(--muted);font-size:12px;margin-top:9px}.stat{font-size:29px;font-weight:900;margin-top:4px;letter-spacing:-1px}.kpi{font-size:11px;color:var(--muted);margin-top:7px}.card{background:rgba(12,25,41,.94);border:1px solid var(--line);border-radius:19px;padding:20px;margin:14px 0;box-shadow:var(--shadow)}.card h2,.card h3{margin-top:0}.muted{color:var(--muted)}.ok{color:var(--primary)}.bad{color:#ff9bab}.warn{color:var(--warn)}.row{display:flex;gap:9px;align-items:center;flex-wrap:wrap}.actions{display:flex;gap:8px;flex-wrap:wrap}.small{font-size:12px}.table-wrap{overflow:auto}.table{width:100%;border-collapse:separate;border-spacing:0;min-width:720px}.table td,.table th{border-bottom:1px solid #1c344c;padding:12px;text-align:right;vertical-align:middle}.table th{color:#9fb5cb;font-size:11px;background:#0d1b2c}.table tr:hover td{background:rgba(255,255,255,.025)}.badge{display:inline-block;padding:5px 9px;border-radius:999px;background:#172b43;font-size:11px}.badge.ok{background:rgba(53,212,154,.12);color:#72efb7}.badge.bad{background:rgba(255,93,118,.12);color:#ff9bab}.badge.warn{background:rgba(255,193,93,.12);color:#ffd38a}
input,textarea,select{width:100%;background:#081625;color:#fff;border:1px solid #2a4561;border-radius:12px;padding:12px 13px;margin:7px 0 14px;outline:none;font:inherit}input:focus,textarea:focus,select:focus{border-color:var(--primary2);box-shadow:0 0 0 3px rgba(57,169,255,.09)}label{font-size:12px;color:#b9cadd}button,.btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;background:linear-gradient(135deg,var(--primary),#22b982);color:#03130d;border:0;border-radius:11px;padding:11px 15px;font-weight:800;cursor:pointer;text-decoration:none;font-size:12px;transition:.16s}button:hover,.btn:hover{transform:translateY(-1px);filter:brightness(1.04)}.btn.blue{background:linear-gradient(135deg,var(--primary2),#4d83ff);color:white}.btn.dark{background:#152b43;color:#e6f0fa;border:1px solid #2b4762}.danger{background:linear-gradient(135deg,#ff667c,#e83e59)!important;color:#fff!important}.light-theme{--bg:#f2f6fa;--surface:#fff;--surface2:#f8fafc;--line:#d6e0ea;--text:#172333;--muted:#5d6d7e;--shadow:0 12px 35px rgba(20,40,60,.12)}.light-theme .sidebar,.light-theme .topbar{background:rgba(255,255,255,.88)}.light-theme .card,.light-theme .statcard{background:#fff;border-color:var(--line)}.light-theme .nav a{color:#33485d}.light-theme input,.light-theme textarea,.light-theme select{background:#fff;color:#172333;border-color:#b9c8d7}.light-theme .table th{background:#f4f7fa}.light-theme .logout,.light-theme .pill{background:#f7f9fb;color:#34495d}.flash{background:#11263a;border:1px solid #284763;padding:11px 13px;margin-bottom:10px;border-radius:12px}.empty{text-align:center;padding:35px;color:var(--muted)}.mini{font-size:11px;color:#7890a9;word-break:break-all}.switch{display:flex;align-items:center;justify-content:space-between;padding:14px;background:#0a1829;border:1px solid var(--line);border-radius:13px;margin:10px 0}.checkgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px}.check{padding:13px;border:1px solid #29425c;border-radius:13px;background:#0a1829}.check input{width:auto;margin:0 7px 0 0}.login-wrap{min-height:100vh;display:grid;place-items:center;padding:25px;background:radial-gradient(circle at 50% 0%,rgba(57,169,255,.16),transparent 35%),var(--bg)}.login{width:min(440px,100%);margin:0}.login-card{text-align:center}.login-card .logo{margin:0 auto 12px}.login-card button{width:100%}.dashboard-layout{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(300px,.8fr);gap:16px}.chart{height:260px;display:flex;align-items:flex-end;gap:12px;padding:25px 10px 10px;border-radius:16px;background:linear-gradient(180deg,rgba(255,255,255,.025),transparent)}.bar{flex:1;min-width:22px;position:relative;height:100%;display:flex;align-items:flex-end}.bar i{display:block;width:100%;height:var(--h);min-height:8px;border-radius:10px 10px 4px 4px;background:linear-gradient(180deg,var(--primary2),var(--primary));box-shadow:0 8px 20px rgba(57,169,255,.14)}.bar span{position:absolute;bottom:-24px;left:50%;transform:translateX(-50%);font-size:10px;color:var(--muted);white-space:nowrap}.donut{width:170px;height:170px;border-radius:50%;margin:10px auto;display:grid;place-items:center;background:conic-gradient(var(--primary) 0 55%,var(--primary2) 55% 82%,var(--warn) 82% 100%);position:relative}.donut:after{content:"";position:absolute;inset:27px;border-radius:50%;background:var(--surface)}.donut b{position:relative;z-index:1;font-size:25px}.legend{display:grid;gap:8px;margin-top:14px}.legend div{display:flex;justify-content:space-between;align-items:center;font-size:11px}.dot{width:9px;height:9px;border-radius:50%;display:inline-block;margin-left:6px}.quick{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.quick a{padding:15px;border:1px solid var(--line);border-radius:14px;background:var(--surface2);text-decoration:none;font-size:12px}.quick a:hover{border-color:#315574}.menu-toggle{display:none}
@media(max-width:1050px){.sidebar-collapsed .app{grid-template-columns:76px minmax(0,1fr)}.app{grid-template-columns:220px minmax(0,1fr)}.dashboard-layout{grid-template-columns:1fr}.content{padding:22px}}
@media(max-width:760px){.app{display:block}.sidebar{position:fixed;right:0;top:0;width:280px;transform:translateX(105%);transition:.22s;box-shadow:-20px 0 50px rgba(0,0,0,.35)}body.menu-open .sidebar{transform:translateX(0)}body.menu-open .sidebar-overlay{display:block;position:fixed;inset:0;background:rgba(0,0,0,.48);backdrop-filter:blur(2px);z-index:20}.sidebar-collapsed .sidebar{width:280px;padding:16px 12px}.sidebar-collapsed .brand>div:not(.logo),.sidebar-collapsed .side-label,.sidebar-collapsed .nav a span{display:block}.sidebar-collapsed .nav a{justify-content:flex-start;padding-left:12px;padding-right:12px}.sidebar-collapsed .logout{font-size:13px;padding:10px 12px}.sidebar-collapsed .logout::after{content:none}.sidebar-collapsed .sidebar-toggle{left:10px}.menu-toggle{display:inline-flex}.topbar{padding:0 14px}.content{padding:18px 12px 35px}.grid{grid-template-columns:repeat(2,minmax(0,1fr))}.hero{padding:20px}.card{padding:16px}.quick{grid-template-columns:1fr}.chart{gap:7px}}
@media(max-width:440px){.grid{grid-template-columns:1fr}.brand h1{font-size:18px}}
</style></head>
<body data-theme="{{theme}}" class="{{'light-theme' if theme=='light' else ''}}">
<canvas id="cursorEffects" class="cursor-effects" aria-hidden="true"></canvas>
...
<div id="commandPalette" class="command-palette" role="dialog" aria-modal="true" aria-label="جستجوی سریع">
  <div class="row" style="justify-content:space-between;margin-bottom:8px"><b>🔎 جستجوی سریع</b><button class="btn dark" type="button" onclick="closeCommandPalette()">✕</button></div>
  <input id="commandInput" type="search" autocomplete="off" placeholder="نام کاربر، آیدی، محصول یا شماره سفارش...">
  <div id="cmdResults" class="cmd-results"></div>
</div>
<div id="notifyPanel" class="notify-panel" role="dialog" aria-label="اعلان‌ها">
  <div class="row" style="justify-content:space-between;margin-bottom:8px"><b>🔔 اعلان‌ها</b><button class="btn dark" type="button" onclick="document.getElementById('notifyPanel').classList.remove('show')">✕</button></div>
  <div id="notifyBody" class="cmd-results" style="max-height:55vh"></div>
</div>
<div id="pageSkeleton" class="skeleton" aria-hidden="true"></div>
<script>
(function(){
  const KEY='iranbot_sidebar_collapsed';
  ...
  window.openCommandPalette=function(){const p=document.getElementById('commandPalette');p.classList.add('show');const i=document.getElementById('commandInput');i.focus();i.value='';i.dispatchEvent(new Event('input'));};
  window.closeCommandPalette=function(){document.getElementById('commandPalette').classList.remove('show');};
  ...
  if(cmdInput){cmdInput.addEventListener('keydown',function(e){if(e.key==='Enter'&&this.value.trim()){location.href='{{url_for("global_search")}}?q='+encodeURIComponent(this.value.trim());}});cmdInput.addEventListener('input',function(){const q=this.value.trim().toLowerCase();const box=document.getElementById('cmdResults');const links=[...document.querySelectorAll('.nav a')].map(a=>({t:a.innerText.trim(),h:a.href})).filter(x=>!q||x.t.toLowerCase().includes(q));box.innerHTML=links.map(x=>`<a href="${x.h}">${x.t}</a>`).join('')||'<div class="empty">برای جستجوی کاربر، محصول یا سفارش Enter بزن.</div>';});}
  window.toggleNotifications=function(){const p=document.getElementById('notifyPanel');p.classList.toggle('show');if(p.classList.contains('show'))loadNotifications();};
  async function loadNotifications(){try{const r=await fetch('{{url_for("live_data")}}',{headers:{'X-Requested-With':'XMLHttpRequest'},cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();const pending=Number(d.pending||0);const prev=Number(localStorage.getItem('iranbot_pending_count')||pending);const count=document.getElementById('notifCount');const body=document.getElementById('notifyBody');if(count)count.textContent=pending;if(pending>prev&&localStorage.getItem('iranbot_sound')!=='0'){try{const ac=new(window.AudioContext||window.webkitAudioContext)();const o=ac.createOscillator(),g=ac.createGain();o.frequency.value=740;g.gain.value=.035;o.connect(g);g.connect(ac.destination);o.start();o.stop(ac.currentTime+.12);}catch(e){}}localStorage.setItem('iranbot_pending_count',String(pending));if(body)body.innerHTML=(d.items||[]).length?(d.items||[]).map(x=>`<div class="notify-item"><b>${x.title||'اعلان'}</b><br><span class="muted">${x.meta||''}</span></div>`).join(''):'<div class="empty">اعلان جدیدی نیست.</div>';}catch(e){const body=document.getElementById('notifyBody');if(body)body.innerHTML='<div class="empty">خطا در دریافت اعلان‌ها. دوباره تلاش کن.</div>';}}
  setInterval(loadNotifications,15000);loadNotifications();
  document.querySelectorAll('a').forEach(a=>a.addEventListener('click',e=>{if(a.origin===location.origin&&!a.target&&a.getAttribute('href')&&!a.getAttribute('href').startsWith('#'))document.getElementById('pageSkeleton').classList.add('show');}));
  const widgets=[...document.querySelectorAll('.dashboard-widget')];widgets.forEach(w=>{w.draggable=true;w.addEventListener('dragstart',()=>w.classList.add('dragging'));w.addEventListener('dragend',()=>{w.classList.remove('dragging');localStorage.setItem('iranbot_widget_order',JSON.stringify([...w.parentElement.children].map(x=>x.dataset.widget).filter(Boolean)));});w.addEventListener('dragover',e=>{e.preventDefault();const d=document.querySelector('.dashboard-widget.dragging');if(d&&d!==w){const r=w.getBoundingClientRect();w.parentElement.insertBefore(d,e.clientY>r.top+r.height/2?w.nextSibling:w);}});});
  if(document.body.dataset.theme==='auto'){const mq=matchMedia('(prefers-color-scheme:light)');const apply=()=>document.body.classList.toggle('light-theme',mq.matches);mq.addEventListener?.('change',apply);apply();}

  // Global electric cursor trail + click lightning, without changing nav underlines.
  const canvas=document.getElementById('cursorEffects');
  ...
  if(ctx){
    ...
    addEventListener('pointerdown',function(e){
      ...
      sparks.push({x:e.clientX,y:e.clientY,t:now});
    },{passive:true});
    function draw(now){
      ctx.clearRect(0,0,w,h);
      const alive=trail.filter(p=>now-p.t<430); trail.splice(0,trail.length,...alive);
      if(trail.length>1){
        ctx.lineCap='round'; ctx.lineJoin='round';
        for(let pass=0;pass<2;pass++){
          ctx.beginPath();
          for(let i=1;i<trail.length;i++){
            const a=trail[i-1],b=trail[i], age=(now-b.t)/430, alpha=Math.max(0,1-age)*(pass?0.28:0.7);
            if(i===1) ctx.moveTo(a.x,a.y);
            const jitter=pass?Math.sin(i*4.7+now*.025)*2:0;
            ctx.lineTo(b.x+jitter,b.y-jitter);
            ctx.strokeStyle=pass?'rgba(57,169,255,'+alpha+')':'rgba(115,225,255,'+alpha+')';
            ctx.lineWidth=pass?7:1.8;
          }
          ctx.stroke();
        }
      }
      for(let i=sparks.length-1;i>=0;i--){
        const s=sparks[i], age=(now-s.t)/520;
        if(age>=1){sparks.splice(i,1);continue;}
        const r=8+age*42, alpha=1-age;
        ctx.beginPath(); ctx.arc(s.x,s.y,r*.34,0,Math.PI*2);
        ctx.fillStyle='rgba(130,235,255,'+(alpha*.32)+')'; ctx.fill();
        for(let k=0;k<12;k++){
          const ang=(Math.PI*2*k/12)+(k%2)*.08, len=r*(.7+(k%3)*.18);
          ctx.beginPath(); ctx.moveTo(s.x+Math.cos(ang)*r*.22,s.y+Math.sin(ang)*r*.22);
          ctx.lineTo(s.x+Math.cos(ang)*len,s.y+Math.sin(ang)*len);
          ctx.strokeStyle='rgba(150,240,255,'+(alpha*.9)+')'; ctx.lineWidth=1.5; ctx.stroke();
        }
      }
  ...
  document.querySelectorAll('.nav a').forEach(function(link){
    link.addEventListener('click', function(e){
      ...
      ripple.style.right=(rect.right-e.clientX-size/2)+'px';
...
{% else %}<main class="login-wrap">{% with msgs=get_flashed_messages() %}{% for m in msgs %}<div class="flash" style="position:fixed;top:20px;right:20px;left:20px;max-width:600px;margin:auto">{{m}}</div>{% endfor %}{% endwith %}{{body|safe}}</main>{% endif %}
</body></html>
'''
...
def ensure_schema():
    with db() as c:
        ...
        c.execute("CREATE TABLE IF NOT EXISTS activity_logs(id INTEGER PRIMARY KEY AUTOINCREMENT,admin_id INTEGER,action TEXT NOT NULL,endpoint TEXT DEFAULT '',ip TEXT DEFAULT '',user_agent TEXT DEFAULT '',created_at DATETIME DEFAULT CURRENT_TIMESTAMP)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_activity_logs_created ON activity_logs(created_at DESC)")
...
def log_activity(action, admin_id=None):
    try:
        aid=admin_id if admin_id is not None else session.get('admin_id')
        with db() as c:
            c.execute('INSERT INTO activity_logs(admin_id,action,endpoint,ip,user_agent) VALUES(?,?,?,?,?)',(aid,action,request.path,client_ip(),request.headers.get('User-Agent','')[:300]))
    except Exception:
        pass

def latest_activity(limit=12):
    with db() as c:
        return c.execute('SELECT id,admin_id,action,endpoint,ip,user_agent,created_at FROM activity_logs ORDER BY id DESC LIMIT ?', (int(limit),)).fetchall()
...
def admin_required(fn):
    ...
    def w(*a,**kw):
        ...
        if request.method == 'POST':
            log_activity(f'POST {request.path}')
        return fn(*a,**kw)
    return w
...
def login():
    ...
    if request.method=='POST':
        ...
        session['admin_id']=aid; log_activity('ورود موفق به Web Panel', aid); return redirect(url_for('dashboard'))
    return page(LOGIN,two_factor=two_factor)

@app.route('/logout')
def logout():
    log_activity('خروج از Web Panel')
    session.clear()
    return redirect(url_for('login'))
...
def live_data():
    with db() as c:
        ...
        rows=c.execute("SELECT id,user_id,amount,created_at FROM payments WHERE status='pending' ORDER BY id DESC LIMIT 6").fetchall()
    items=[{'title':f'💳 پرداخت در انتظار #{r[0]}','meta':f'کاربر {r[1]} · {int(r[2]):,} تومان · {r[3]}'} for r in rows]
    return jsonify({'pending':pending,'items':items,'server_time':datetime.now().isoformat(timespec='seconds')})
...
def global_search():
    ...
    like=f'%{q}%'
    with db() as c:
        users=c.execute("SELECT user_id,username,first_name FROM users WHERE CAST(user_id AS TEXT) LIKE ? OR username LIKE ? OR first_name LIKE ? ORDER BY last_seen DESC LIMIT 30",(like,like,like)).fetchall()
        products=c.execute("SELECT id,name,price FROM products WHERE name LIKE ? LIMIT 30",(like,)).fetchall()
        orders=c.execute("SELECT o.id,o.user_id,p.name,o.status FROM orders o JOIN products p ON p.id=o.product_id WHERE CAST(o.id AS TEXT) LIKE ? OR CAST(o.user_id AS TEXT) LIKE ? OR p.name LIKE ? ORDER BY o.id DESC LIMIT 30",(like,like,like)).fetchall()
    b="""<div class='hero'><h2>🔎 جستجوی سراسری</h2><p>نتایج برای «{{q}}»</p></div><div class='card'><h3>👥 کاربران</h3>{% for r in users %}<p><a class='btn dark' href='{{url_for('user_detail_web',uid=r[0])}}'>{{r[0]}} · @{{r[1] or '-'}} · {{r[2] or '-'}}</a></p>{% else %}<div class='empty'>موردی پیدا نشد.</div>{% endfor %}</div><div class='card'><h3>🛒 محصولات</h3>{% for r in products %}<p>#{{r[0]}} · {{r[1]}} · {{r[2]}} تومان</p>{% else %}<div class='empty'>موردی پیدا نشد.</div>{% endfor %}</div><div class='card'><h3>📦 سفارش‌ها</h3>{% for r in orders %}<p>#{{r[0]}} · کاربر {{r[1]}} · {{r[2]}} · {{r[3]}}</p>{% else %}<div class='empty'>موردی پیدا نشد.</div>{% endfor %}</div>"""
    return page(b,q=q,users=users,products=products,orders=orders)

@app.route('/activity-log')
@admin_required
def activity_log():
    rows=latest_activity(200)
    b="""<div class='hero'><h2>🕘 گزارش فعالیت ادمین</h2><p>ورودها، خروج‌ها و عملیات Web Panel همراه با IP ثبت می‌شوند.</p></div><div class='card'><div class='table-wrap'><table class='table'><tr><th>زمان</th><th>ادمین</th><th>عملیات</th><th>IP</th><th>مسیر</th><th>Browser</th></tr>{% for r in rows %}<tr><td>{{r[6]}}</td><td>{{r[1]}}</td><td>{{r[2]}}</td><td dir='ltr'>{{r[4]}}</td><td dir='ltr'>{{r[3]}}</td><td class='mini' dir='ltr'>{{r[5]}}</td></tr>{% else %}<tr><td colspan='6' class='empty'>گزارشی ثبت نشده.</td></tr>{% endfor %}</table></div></div>"""
...
def dashboard():
    with db() as c:
        ...
        revenue=c.execute("SELECT COALESCE(SUM(amount),0) FROM payments WHERE status='approved'").fetchone()[0]
        activity=latest_activity(8)
        panel_rows=c.execute('SELECT id,name,status FROM panels ORDER BY id').fetchall()
        backup_last=setting('backup_last_manual') or setting('backup_weekly_last_sent') or 'هنوز Backup ثبت نشده'
    ...
    <div class="grid" style="margin-bottom:16px">
      ...
      <div class="statcard"><div class="staticon">🛒</div><div class="statlabel">محصولات فعال</div><div class="stat">{{products}}</div><div class="kpi">قابل فروش</div></div>
      <div class="statcard"><div class="staticon">📦</div><div class="statlabel">کل سفارش‌ها</div><div class="stat">{{orders}}</div><div class="kpi">{{paid}} پرداخت موفق</div></div>
      <div class="statcard"><div class="staticon">💰</div><div class="statlabel">واریزی تأییدشده</div><div class="stat">{{revenue|money}}</div><div class="kpi">تومان</div></div>
      <div class="statcard"><div class="staticon">⏳</div><div class="statlabel">در انتظار بررسی</div><div class="stat">{{pending}}</div><div class="kpi">رسید پرداخت</div></div>
      <div class="statcard"><div class="staticon">🖥</div><div class="statlabel">پنل‌ها</div><div class="stat">{{panels}}</div><div class="kpi">پنل‌های ثبت‌شده</div></div>
    </div>
    <div class="dashboard-layout">
      <div>
        <div class="card"><div class="row" style="justify-content:space-between"><div><h3 style="margin-bottom:4px">📈 فعالیت سفارش‌ها</h3><div class="muted small">تعداد سفارش در ۷ روز اخیر</div></div><span class="badge ok">LIVE</span></div><div class="chart">{% for label,height in bars %}<div class="bar" title="{{label}}"><i style="--h:{{height}}%"></i><span>{{label}}</span></div>{% endfor %}</div></div>
        <div class="card"><div class="row" style="justify-content:space-between"><h3>📦 آخرین سفارش‌ها</h3><a class="btn dark" href="{{url_for('orders')}}">مشاهده همه</a></div>{% if recent %}<div class="table-wrap"><table class="table"><tr><th>#</th><th>User</th><th>محصول</th><th>وضعیت</th><th>تاریخ</th></tr>{% for r in recent %}<tr><td>{{r[0]}}</td><td>{{r[1]}}</td><td>{{r[2]}}</td><td><span class="badge {{'ok' if r[3]=='paid' else 'warn'}}">{{r[3]}}</span></td><td>{{r[4]}}</td></tr>{% endfor %}</table></div>{% else %}<div class="empty">هنوز سفارشی ثبت نشده.</div>{% endif %}</div>
      </div>
      <div>
        <div class="card"><h3>⚡ دسترسی سریع</h3><div class="quick"><a href="{{url_for('products')}}">🛒<br><b>محصولات</b><br><span class="muted">مدیریت فروش</span></a><a href="{{url_for('finance_report_web')}}">📊<br><b>گزارش مالی</b><br><span class="muted">بررسی درآمد</span></a><a href="{{url_for('users')}}">👥<br><b>کاربران</b><br><span class="muted">مدیریت حساب‌ها</span></a><a href="{{url_for('panels')}}">🖥<br><b>پنل‌ها</b><br><span class="muted">اتصال و Group</span></a></div></div>
        <div class="card"><h3>📊 وضعیت سفارش‌ها</h3><div class="donut"><b>{{paid}}</b></div><div class="legend"><div><span><i class="dot" style="background:var(--primary)"></i>پرداخت موفق</span><b>{{paid}}</b></div><div><span><i class="dot" style="background:var(--primary2)"></i>کل سفارش‌ها</span><b>{{orders}}</b></div><div><span><i class="dot" style="background:var(--warn)"></i>رسیدهای در انتظار</span><b>{{pending}}</b></div></div></div>
      </div>
    </div>
    <div class="dashboard-layout"><div class="card dashboard-widget" data-widget="activity"><h3>🕘 فعالیت اخیر</h3>{% for r in activity %}<div class="notify-item"><b>{{r[2]}}</b> · ادمین {{r[1]}} · <span dir="ltr">{{r[4]}}</span><br><span class="muted">{{r[6]}}</span></div>{% else %}<div class="empty">فعالیتی ثبت نشده.</div>{% endfor %}</div><div class="card dashboard-widget" data-widget="health"><div class="row" style="justify-content:space-between"><h3>🖥 سلامت پنل‌ها</h3><a class="btn dark" href="{{url_for('panels')}}">مدیریت</a></div>{% for p in panel_rows %}<div class="row" style="justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--line)"><span>{{p[1]}}</span><span class="badge {{'ok' if p[2]=='ok' else 'warn'}}">{{p[2] or 'unknown'}}</span></div>{% else %}<div class="empty">پنلی ثبت نشده.</div>{% endfor %}<p class="small muted">💾 آخرین Backup: {{backup_last}}</p></div></div>'''
    return page(b,users=users,products=products,orders=orders,paid=paid,pending=pending,panels=panels,revenue=revenue,recent=recent,bars=bars,activity=activity,panel_rows=panel_rows,backup_last=backup_last)
...
def backup():
    ...
    manual=setting('backup_last_manual') or 'هنوز ساخته نشده'
    b="""<div class='hero'><h2>💾 پشتیبان‌گیری و بازیابی</h2><p>یک فایل کامل از اطلاعات Database بساز، دانلود کن یا در صورت نیاز Backup قبلی را برگردان.</p></div><div class='card'><h3>📥 گرفتن فایل Backup</h3><p class='small muted'>آخرین Backup دستی: {{manual}}</p><p class='muted'>کل اطلاعات ذخیره‌شده در SQLite، شامل کاربران، کیف پول‌ها، پنل‌ها، Groupها، محصولات، سفارش‌ها، پرداخت‌ها، تنظیمات، تیکت‌ها و سایر جدول‌های Database در یک فایل ذخیره می‌شود.</p><a class='btn blue' href='{{url_for('backup_download')}}'>💾 ساخت و دانلود Backup</a></div><div class='card'><h3>📤 وارد کردن فایل Backup</h3><p class='muted'>فایل Backup قبلی را انتخاب کن. فایل قبل از بازیابی از نظر SQLite و جدول‌های اصلی بررسی می‌شود.</p><form method='post' action='{{url_for('backup_restore')}}' enctype='multipart/form-data'><input type='file' name='backup_file' accept='.db,.sqlite,.sqlite3' required><button class='danger'>📤 بازیابی Backup</button></form></div><div class='card'><div class='row' style='justify-content:space-between'><div><h3>📅 Backup خودکار هر 7 روز</h3><p class='muted'>وقتی فعال باشد، Bot هر 7 روز یک فایل کامل Backup برای همه ADMIN_IDS ارسال می‌کند.</p></div><span class='badge {{'ok' if enabled else 'bad'}}'>{{'🟢 فعال' if enabled else '🔴 غیرفعال'}}</span></div><p class='small muted'>آخرین ارسال: {{last}}</p><form method='post'><button name='action' value='{{'weekly_off' if enabled else 'weekly_on'}}' class='{{'danger' if enabled else ''}}'>{{'2️⃣ غیرفعال کردن' if enabled else '1️⃣ فعال کردن'}}</button></form></div>"""
    return page(b,enabled=enabled,last=last,manual=manual)
...
def backup_download():
    ...
    try:
        ...
        set_setting('backup_last_manual', datetime.now().isoformat(timespec='seconds'))
        log_activity('ساخت و دانلود Backup')
