...
(function(){
  ...
  async function loadNotifications(){try{const r=await fetch('{{url_for("live_data")}}');const d=await r.json();const prev=Number(localStorage.getItem('iranbot_pending_count')||d.pending);document.getElementById('notifCount').textContent=d.pending;if(d.pending>prev&&localStorage.getItem('iranbot_sound')!=='0'){try{const ac=new(window.AudioContext||window.webkitAudioContext)();const o=ac.createOscillator(),g=ac.createGain();o.frequency.value=740;g.gain.value=.035;o.connect(g);g.connect(ac.destination);o.start();o.stop(ac.currentTime+.12);}catch(e){}}localStorage.setItem('iranbot_pending_count',d.pending);document.getElementById('notifyBody').innerHTML=d.items.length?d.items.map(x=>`<div class="notify-item"><b>${x.title}</b><br><span class="muted">${x.meta}</span></div>`).join(''):'<div class="empty">اعلان جدیدی نیست.</div>';}catch(e){}}
