...
a{color:inherit}.app{display:grid;grid-template-columns:250px 1fr;min-height:100vh}.sidebar{position:sticky;top:0;height:100vh;padding:20px 14px;background:rgba(7,17,31,.88);border-left:1px solid var(--line);backdrop-filter:blur(18px);z-index:10}.brand{display:flex;align-items:center;gap:11px;padding:6px 8px 20px;border-bottom:1px solid var(--line);margin-bottom:15px}.logo{width:46px;height:46px;border-radius:15px;display:grid;place-items:center;background:linear-gradient(135deg,var(--primary2),var(--primary));box-shadow:0 10px 35px rgba(57,169,255,.24);font-size:23px}.brand h1{font-size:20px;margin:0}.version{display:block;color:var(--muted);font-size:11px;margin-top:3px}.side-label{font-size:10px;color:#67809a;padding:8px 10px;text-transform:uppercase}.nav{display:flex;flex-direction:column;gap:5px}.nav a{text-decoration:none;color:#cfe0f1;padding:11px 12px;border-radius:13px;border:1px solid transparent;display:flex;align-items:center;gap:9px;transition:.18s;font-size:13px}.nav a:hover{background:var(--surface2);border-color:var(--line);transform:translateX(-2px)}.nav a.active{background:linear-gradient(135deg,rgba(57,169,255,.18),rgba(53,212,154,.12));border-color:#285273;color:#fff}.side-bottom{position:absolute;bottom:18px;left:14px;right:14px}.logout{display:block;text-align:center;text-decoration:none;padding:10px 12px;border:1px solid var(--line);background:var(--surface2);border-radius:12px;color:#dbe8f5;font-size:13px}
...
@media(max-width:1050px){.app{grid-template-columns:210px 1fr}.dashboard-layout{grid-template-columns:1fr}.content{padding:22px}}
@media(max-width:760px){.app{display:block}.sidebar{position:fixed;right:0;top:0;width:280px;transform:translateX(105%);transition:.22s;box-shadow:-20px 0 50px rgba(0,0,0,.35)}body.menu-open .sidebar{transform:translateX(0)}.menu-toggle{display:inline-flex}.topbar{padding:0 14px}.content{padding:18px 12px 35px}.grid{grid-template-columns:repeat(2,minmax(0,1fr))}.hero{padding:20px}.card{padding:16px}.quick{grid-template-columns:1fr}.chart{gap:7px}}
...
<aside class="sidebar"><div class="brand"><div class="logo">⚡</div><div><h1>IRANBOT</h1><span class="version">پنل مدیریت · v{{version}}</span></div></div>
