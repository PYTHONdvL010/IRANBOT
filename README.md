...
> فروشگاه حرفه‌ای سرویس VPN و کانفیگ در Telegram با پنل مدیریت Web، پرداخت، کیف پول، تست رایگان، سیستم کد تخفیف، بخش قرعه‌کشی و اتصال به Marzban / Pasarguard / 3x-ui.
...
# ✨ تغییرات و امکانات اضافه‌شده در نسخه 1.0.7

نسخه **1.0.7** تمام امکانات نسخه 1.0.6 را حفظ می‌کند و موارد زیر را اضافه می‌کند.
...
در منوی اصلی Web Panel نیز قرار گرفته است.
...
نسخه **1.0.7** تمام امکانات قبلی نسخه 1.0.6 را بدون حذف حفظ می‌کند.
...
بنابراین اطلاعات کاربران، کیف پول، پنل‌ها، Group ها، محصولات، سفارش‌ها، پرداخت‌ها، تنظیمات، تیکت‌ها، تست‌های رایگان و کدهای تخفیف داخل Backup قرار می‌گیرند.
...
14. ساخت Product
        ↓
15. ساخت Discount در صورت نیاز
        ↓
16. تست خرید
        ↓
17. فعال‌سازی Backup هفتگی
```
...
> این README مربوط به **نسخه 1.0.7** است. امکانات نسخه 1.0.6 حفظ شده‌اند و در نسخه 1.0.7 سیستم کد تخفیف و بخش قرعه‌کشی اضافه شده است. بخش قرعه‌کشی در این نسخه هنوز در حال ساخت است و منطق کامل آن فعال نشده است.
